...
```
Everything seems to have been read and present. 
...
summary(d$fracinf)
d %>%
  ggplot(aes(fracinf, RateAll)) + geom_point()

#We plotted the data against RateAll and we can see that they have a surprisingly linear relatioship. This may indicate a relationship between these variables. 
...
#ggplot will automatically remove NA values, a total of 120 rows at this time. We also have both variables in the same units.

# also plot the difference of the 2 variables
difference <- d$RateAll - d$fracinf
ggplot(d, aes(RiskAll, difference)) + geom_point()

#This shows that the most difference between the variables occurs when RiskAll equals 0. 
...
moving_forward <- better %>%
  dplyr::select(Action1, CasesAll, Category, Country, Deaths, gg2c4, Hemisphere, Hospitalizations, MeanA1, MeanD1, MeanI1, MedianA1, MedianD1, MedianI1, OBYear, Path1, RiskAll, season, Setting_1, Trans1, Vomit, fracinf)

str(moving_forward)
summary(moving_forward)

#We can see we have 579 observations and 22 variables.
...
almost_there <- moving_forward %>%
  dplyr::select(Action1, CasesAll, Category, Country, Deaths, gg2c4, Hemisphere, Hospitalizations, MeanD1, MeanI1, MedianD1, MedianI1, OBYear, Path1, RiskAll, season, Setting_1, Trans1, Vomit, fracinf)

almost_there <- na.omit(almost_there)

str(almost_there)
summary(almost_there)
...
vis_dat(almost_there)

#None of our variables are coded as characters and seemed to all be coded appropriately.
```
...
addlevel <- levels(almost_there$Setting_1) 
Restaurant <- grep("Restaurant", almost_there$Setting_1, ignore.case = TRUE, value=TRUE)
Other <- grep("Restaurant", almost_there$Setting_1, ignore.case = TRUE, value=TRUE,
                  invert=TRUE)
Final <- almost_there %>%
  mutate(Setting = fct_collapse(Setting_1, Restaurant = restaurantlist, Other = othlist))
```
...
final %>% select_if(is.numeric)->plots1
plots1.m = reshape2::melt(plots1, id.var="fracinf")
plots1.m %>%
...
which(final$OBYear == 0)
finalyear <- final[-219,] 
finalyear %>% select_if(is.numeric)->num0
...
CorrPlot <- cor(num0)
#corrplot(CorrPlot, method="numeric")
...
finalyear %>% select_if(is.factor)->catpredict
finalyear['fracinf'] -> catpredict['fracinf']
catpredict.m = reshape2::melt(catpredict, id.var="fracinf")
ggplot(catpredict.m)+
...
finalhemisphere <- finalcategory %>%
  subset(Hemisphere != "Unspecified")

finalcombine <- finalhemisphere %>%
  mutate(Action1= fct_recode(Action1, "Unspecified" = "Unknown"))

finalcombine2 <- finalcombine %>%
  mutate(Path1= fct_recode(Path1, "Unspecified" = "Unknown"))

glimpse(finalcombine2)

#We have a dataframe containing 551 observations and 19 variables. 

finalcombine2 %>% select_if(is.factor)->catpredict
finalcombine2['fracinf'] -> catpredict['fracinf']
catpredict.m = reshape2::melt(catpredict, id.var="fracinf")
ggplot(catpredict.m)+
  geom_dotplot(aes(x = value, fill = fracinf))+
  facet_wrap(~variable)
...
set.seed(123)
trainset <- caret::createDataPartition(y = finalcombine2$fracinf, p = 0.7, list = FALSE)
data_train = finalcombine2[trainset,] #extract observations/rows for training, assign to new variable
data_test = finalcombine2[-trainset,] #do the same for the test set
```
...
#Root mean squared error is 28.752. 
```
...
data_train2 <- data_train %>%
  select(-c(Deaths, Hemisphere, MeanD1, MeanI1, MedianD1, MedianI1))
...
fit1 <- train(fracinf ~ ., data= data_train2, method= "lm", trControl= fitControl, preProcess= c("center","scale"))
fit2 <- train(fracinf ~ ., data= data_train2, method= "earth", trControl= fitControl, preProcess= c("center","scale"))
fit3 <- train(fracinf ~ ., data= data_train2, method= "knn", trControl= fitControl, preProcess= c("center","scale"))
print(fit1)
print(fit2)
print(fit3)
```
...
uncert <- resamples(list(Lm= fit1, earth= fit2, knn= fit3))
diffs <- diff(uncert)
diffs
summary(diffs)
densityplot(diffs)
...
predicted <- predict(fit3, newdata= data_train, interval="confidence")
addpredict <- data_train2 %>%
  mutate(predicted= predicted)


tresid <- residuals(fit3)


ggplot(addpredict, aes(predicted, data_train2$fracinf)) + geom_point() + geom_smooth(method="lm")
RMSE(predicted, data_train2$fracinf)
ggplot(addpredict, aes(predicted, tresid)) + geom_point() + geom_smooth(method="lm")
...
set.seed(1111)
finalmod <- predict(fit3, newdata= data_test, interval="confidence")
finalmod_predict <- data_test %>%
  mutate(Final_KNN_predictions= finalmod)


ggplot(finalmod_predict, aes(finalmod, data_test$fracinf)) + geom_point() + geom_smooth(method="lm")


RMSE(data_test$fracinf, finalmod)
